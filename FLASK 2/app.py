...
# TODO ALUNO: importe SQLAlchemy (Importados também os tipos de dados necessários para o Model)
...
def index():
    ...
    # No SQLAlchemy 2.0, a sintaxe abaixo substitui o antigo 'Produto.query'
    produtos = db.session.execute(db.select(Produto).order_by(Produto.nome)).scalars().all()
